...
from .model import GaussianPolicy, QNetworkCont, DeterministicPolicy
...
class SACTrainer(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.critic = QNetworkCont(num_inputs, action_shape, args.hidden_dim).to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=args.critic_lr)

        self.critic_target = QNetworkCont(num_inputs, action_shape, args.hidden_dim).to(self.device)
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(num_inputs, action_shape, args.hidden_dim, action_space).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=args.policy_lr)

        else:
            self.alpha = 0
            self.automatic_entropy_tuning = False
            self.policy = DeterministicPolicy(num_inputs, action_shape, args.hidden_dim, action_space).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=args.policy_lr)
    ...
    def act(self, state, eval=False):
        ...
        return action.detach().cpu().numpy()[0], np.array([1.0])
